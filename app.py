...
def create_map_with_drawing(center_lng, center_lat, waypoints, home_point, obstacles, coord_system):
    ...
    for i, obstacle in enumerate(obstacles):
        ...
        folium.Polygon(
            ...
            fill=True,
            fill_opacity=0.4,
            popup=f"🚧 {obstacle['name']}\n高度：{obstacle['height']}米",
            tooltip=f"{obstacle['name']} | 高{obstacle['height']}m"
        ).add_to(m)
...
if 'last_draw_hash' not in st.session_state:
    st.session_state.last_draw_hash = ""
if 'temp_obstacle_points' not in st.session_state:
    st.session_state.temp_obstacle_points = None
...
with st.sidebar:
    ...
    if "🗺️ 航线规划" in st.session_state.page:
        ...
        # 【新增】圈选后高度设置表单
        if st.session_state.temp_obstacle_points is not None:
            st.warning("⚠️ 请设置障碍物高度并保存")
            obs_height = st.number_input("障碍物高度（米）", min_value=1, max_value=500, value=20, step=1, key="new_obs_height")
            ...
            with col_save:
                if st.button("✅ 保存障碍物", key="save_obs"):
                    new_obstacle = {
                        ...
                        'height': obs_height,
                        'points': st.session_state.temp_obstacle_points,
                        'created_at': get_beijing_time().strftime("%Y-%m-%d %H:%M:%S")
                    }
        ...
        if st.session_state.obstacles:
            obs_options = [f"{i+1}. {o['name']} (高{o['height']}m)" for i, o in enumerate(st.session_state.obstacles)]
...
if "飞行监控" in st.session_state.page:
    ...
    if heartbeats:
        ...
        if len(heartbeats)>=2:
            ...
            fig_interval.add_trace(go.Scatter(x=seqs, y=intervals_data, mode='lines+markers', name='心跳间隔', line=dict(color='orange',width=2), marker=dict(size=6,color='red')))
        ...
        # 趋势图
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df['time'], y=df['seq'], mode='lines+markers', name='心跳', line=dict(color='blue',width=2), marker=dict(size=6,color='red')))
        ...
        display_df.columns = ['北京时间(毫秒级)', '序列号']
        st.dataframe(display_df, use_container_width=True)
        
        latest = heartbeats[-1]
        st.success(f"✅ 最新心跳: {latest['time_ms']} | 序列号: {latest['seq']}")
        now_beijing = get_beijing_time()
        st.caption(f"🕐 当前时间: {now_beijing.strftime('%Y年%m月%d日 %H:%M:%S')}")
    else:
        st.info("等待心跳数据...")

else:
    st.header("🗺️ 航线规划 - 南京科技职业学院")
    st.caption("🎨 使用右上角多边形工具绘制障碍物 → 设置高度 → 保存")
    ...
    with col2:
        if st.session_state.waypoints:
            st.success(f"✈️ 航线：教学楼 → 操场")
            st.success(f"航点数：{len(st.session_state.waypoints)}")
        else:
            st.warning("⚠️ 未生成航线")
        st.info(f"🚧 障碍物总数：{len(st.session_state.obstacles)}")
    ...
    with st.spinner("加载高德卫星地图..."):
        try:
            if st.session_state.waypoints:
                all_p = [st.session_state.home_point] + st.session_state.waypoints
                c_lng = sum(p[0] for p in all_p)/len(all_p)
                c_lat = sum(p[1] for p in all_p)/len(all_p)
            else:
                c_lng, c_lat = st.session_state.home_point
            ...
            if output and output.get('last_draw'):
                ...
                current_hash = str(draw_data)
                
                if current_hash != st.session_state.last_draw_hash:
                    st.session_state.last_draw_hash = current_hash
            ...
            st.info("🎨 使用地图右上角多边形图标绘制，双击完成 → 去左侧设置高度保存")
    ...
    if st.session_state.obstacles:
        ...
        for i, obs in enumerate(st.session_state.obstacles):
            with st.expander(f"障碍物 {i+1}：{obs['name']}（高{obs['height']}米）"):
                st.write(f"创建时间：{obs['created_at']}")
                st.write(f"顶点数量：{len(obs['points'])}")
                for j, p in enumerate(obs['points']):
                    st.write(f"点{j+1}：{p[0]:.6f}, {p[1]:.6f}")
    ...
    if len(st.session_state.waypoints)>=2:
        ...
        with c1: st.metric("起点A", f"{a[0]:.6f},{a[1]:.6f}")
        with c2: st.metric("终点B", f"{b[0]:.6f},{b[1]:.6f}")
        with c3: st.metric("直线距离", f"{dist:.1f}米")
