...
with st.sidebar:
    ...
    if "🗺️ 航线规划" in st.session_state.page:
        ...
        # 输入框绑定 session_state
        new_name = st.text_input("障碍物名称", value=st.session_state.temp_name, key="new_name")
        new_height = st.number_input("高度(米)", min_value=1, max_value=200, value=st.session_state.temp_height, step=5, key="new_height")
...
if "飞行监控" in st.session_state.page:
    ...
    st.caption("🎨 使用地图右上角的多边形工具绘制障碍物，绘制后双击完成，然后在左侧输入名称和高度，点击「保存障碍物」")
