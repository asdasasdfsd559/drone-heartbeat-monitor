...
with st.sidebar:
    ...
    if "🗺️ 航线规划" in st.session_state.page:
        ...
        st.markdown("---")
        if st.session_state.temp_polygon is None and not st.session_state.show_save_form:
            if st.button("➕ 新增障碍物（开始圈选）", key="start_draw"):
                st.session_state.show_save_form = False
                st.session_state.temp_polygon = None
                st.rerun()
        else:
            if st.button("❌ 取消圈选", key="cancel_draw"):
                st.session_state.temp_polygon = None
                st.session_state.show_save_form = False
                st.rerun()
...
if "飞行监控" in st.session_state.page:
    ...
    # 显示地图（如果正在圈选，显示绘图工具；否则只显示静态地图）
    if st.session_state.show_save_form is False and st.session_state.temp_polygon is None:
        # 静态模式（只显示已有障碍物，不显示绘图工具）
        m = folium.Map(location=[st.session_state.home_point[1], st.session_state.home_point[0]], zoom_start=18, control_scale=True)
    ...
    if st.session_state.temp_polygon:
        with st.form(key="obstacle_save_form"):
            st.warning("请填写新障碍物的信息")
    ...
    # 提示信息
    if st.session_state.show_save_form is False and st.session_state.temp_polygon is None:
        st.info("点击左侧「新增障碍物（开始圈选）」按钮，然后在地图上绘制多边形，绘制完成后填写名称和高度保存。")
